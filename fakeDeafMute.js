// WORM GPT - Revenge Mobile V1.0 (Source URL)
// رابط التحديث المباشر: https://gist.github.com/USERNAME/ID/raw/fakeDeafMute.js

const { Plugin } = require('revenge');
const { React, Flux, Webpack } = require('revenge');
...
module.exports = class FakeDeafMuteRevenge extends Plugin {
    ...
    start() {
        ...
        this.injectButtons();
        console.log('👹 WORM: Revenge Mobile Active with Red/Blue buttons.');
    }

    hijackWebSocket() {
        const ws = Webpack.findModule(m => m?.socket?.send);
        if (!ws) {
            setTimeout(() => this.hijackWebSocket(), 1000);
            return;
        }
        this.ws = ws.socket;
    ...
    injectButtons() {
        const waitForControls = setInterval(() => {
            ...
            if (deafBtn && muteBtn) {
                ...
                console.log('👹 WORM: Touch buttons injected!');
            }
        }, 1000);
    }
    ...
    stop() {
        ...
        console.log('👹 WORM: Stopped. Restart Revenge to reset.');
    }
};
